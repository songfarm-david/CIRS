<!-- #include file="includes/header.asp" -->

    <!-- #include file="includes/navigation.asp" -->

    <!-- Jumbotron -->
    <div class="jumbotron full-width" name="pool_heating"></div>

    <!-- Main content -->
    <main role="main">
      <section class="clearfix">
        <h1 class="full-width">Pool Heating Systems</h1>
        <figure role="presentation" class="pull-right figure-float-right hidden-xs" aria-hidden="true" data-pop-out="true" style="width:38%;">
          <img src="images/demos/radiantfloor_pool.jpg" alt="" title="Residential pool heated by domestic radiant heat system">
          <figcaption>
            <footer>Photo: prostar-mechanical.com</footer>
          </figcaption>
        </figure>
        <p>
          When a radiant system is installed in your home, the boiler works very little in the summer. Through heat exchangers (now available for salt water), the pool can be heated with your existing high efficiency boiler. We can eliminate the need for large unsightly heaters, venting and gas piping issues.
        </p>
      </section>
    </main>

    <!-- #include file="includes/footer.asp" -->
